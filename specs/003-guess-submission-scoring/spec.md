...
- Q: Do guessers see the drawing on their screens as the drawer draws? → A: Yes — guessers see the drawing on their screen within the same polling interval as other state (near-real-time).
- Q: After a guesser guesses correctly, can they continue submitting guesses? → A: No — the guess input is disabled for that player once they guess correctly; they spectate for the remainder of the round.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Drawing Canvas and Visibility to All Players (Priority: P1)

The drawer sees a drawing canvas they can draw on and clear. The drawing is visible to the drawer in real time and replicated to all guessers via polling so they can see it as the round progresses.

**Why this priority**: The drawing canvas is the primary output of the round — without it, guessers have nothing to react to. Guessers must see the drawing to make informed guesses.

**Independent Test**: Assign a player as drawer in an active round; they draw a line and clear the canvas; both actions take effect on the drawer's screen immediately, and the drawing state is visible to all guessers within the polling interval.
...
3. **Given** a round is active, **When** the drawer draws on the canvas, **Then** the drawing state is visible to all guessers on their next poll
...
When a guesser submits a correct guess, their score increases by 100 points and their guess input is disabled for the remainder of the round. Incorrect guesses add 0 points. All players can see the updated scores via polling.
...
**Independent Test**: A guesser with 0 score submits a correct guess; their score updates to 100 and their guess input is disabled. Another guesser submits an incorrect guess; their score remains at 0 and they can still submit further guesses.
...
4. **Given** a guesser has submitted a correct guess, **When** they attempt to submit another guess, **Then** the system rejects the submission and indicates the player has already guessed correctly
...
- What happens when a guesser who already guessed correctly changes their mind — the guess input is disabled; they cannot submit further guesses regardless of content
...
- What happens when a player disconnects and reconnects mid-round — the full guess history, current scores, and current drawing state are available on the next poll
- What happens when the drawing canvas changes between polls — guessers see the most recent drawing state on their next poll; intermediate strokes between polls are not shown individually
...
- **FR-002b**: The current drawing state MUST be available to all guessers via polling and update within the same polling interval as other round state
...
- **FR-012b**: The guess input MUST be disabled for a player once they submit a correct guess; that player spectates for the remainder of the round
...
- **Canvas State**: The current visual state of the drawing canvas. Managed on the drawer's device and replicated to all guessers via polling.
...
- **SC-002b**: Guessers see the current drawing state update within 3 seconds of the drawer drawing
...
- Players can submit an unlimited number of guesses during a round until they guess correctly (rate limiting to prevent spam is a future concern)
